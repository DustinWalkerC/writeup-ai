...
const PRIMARY_FILE_TYPES: Record<string, { label: string; desc: string; required: boolean }> = {
  t12: {
    label: 'T-12 Operating Statement',
    desc: 'Monthly income and expense statement',
    required: true,
  },
  rent_roll: {
    label: 'Rent Roll',
    desc: 'Current tenant and unit details',
    required: false,
  },
  leasing_activity: {
    label: 'Leasing Activity Report',
    desc: 'Traffic, applications, move-ins/outs',
    required: false,
  },
};
...
export default function FileUploader({ reportId, existingFiles, onFilesChanged }: FileUploaderProps) {
  ...
  const primaryFiles = files.filter(f => f.file_type in PRIMARY_FILE_TYPES);
  const additionalFiles = files.filter(f => !(f.file_type in PRIMARY_FILE_TYPES));
  ...
  const handleUpload = useCallback(async (fileType: string, file: File) => {
    ...
    try {
      ...
      if (fileType in PRIMARY_FILE_TYPES) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {Object.entries(PRIMARY_FILE_TYPES).map(([type, cfg]) => {
