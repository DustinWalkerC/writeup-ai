...
export function ReportViewer({ reportId, report, userSettings }: Props) {
  ...
  const handleDrop = useCallback(
    async (targetIndex: number) => {
      ...
      newOrder.forEach((s, i) => {
        if (newSections[s.id]) newSections[s.id] = { ...newSections[s.id], order: i + 1 }
      })
  ...
  const handleExportHTML = () => {
    ...
    try {
      ...
      downloadHTML(html, getExportFilename('html'))
  ...
  const handleEmailPreview = async () => {
    ...
    showStatus('Building email preview...')
    try {
      const result = await generateEmailHTML(reportId)
      setEmailPreviewHTML(result.html)
      setEmailPreviewSize(result.estimatedSizeKB)
      showStatus(null)
    } catch (err) {
      console.error('Email preview failed:', err)
      showStatus('Email preview failed — check console')
    } finally {
      setEmailPreviewLoading(false)
    }
  }

  // ✅ Edit 1 — keep the function (and ensure it contains the required filename/await lines)
  const handleDownloadEmailHTML = async () => {
    const filename = getExportFilename('html')
    await downloadEmailHTML(reportId, filename)
  }

  // ✅ Edit 1 — Add the copy handler function RIGHT AFTER handleDownloadEmailHTML
  const handleCopyEmailHTML = async () => {
    if (!emailPreviewHTML) return
    try {
      // Copy as text/html so Gmail/Outlook renders it (not raw code)
    ...
    <div style={{ background: W.bg, border: `1px solid ${W.border}`, borderRadius: 14, overflow: 'hidden' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontSize: 13,
                fontWeight: viewMode === tab.mode ? 600 : 500,
          ...
          <div style={{ maxWidth: 960, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ position: 'relative', paddingTop: 4 }}>
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = `${W.accent}50`
                  e.currentTarget.style.color = W.accent
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = W.border
                  e.currentTarget.style.color = W.textSoft
                }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {emailPreviewHTML && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setEmailPreviewHTML(null)
          }}
        >
          <div
            style={{
      ...
      >
        <Link
          ...
          style={{
            ...
            borderRadius: 10,
            textDecoration: 'none',
            boxShadow: `0 2px 12px ${W.accent}30`,
